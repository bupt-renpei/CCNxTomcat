...
import ccntomcat.CCNConnector;
import ccntomcat.testHash;
...
public class CCNSegmenter {
	...
	protected testHash testhash;
	
	public testHash getTesthash() {
		return testhash;
	}

	public void setTesthash(testHash testhash) {
	...
	public CCNSegmenter(CCNFlowControl flowControl, CCNAggregatedSigner signer) {
		...
		this.testhash = _handle.getTesthash();
		if (null == signer) {
	...
	throws InvalidKeyException, InvalidAlgorithmParameterException, IOException, SignatureException, NoSuchAlgorithmException {
		...
		for (int i=0; i < blockCount; ++i) {
			...
			//tu 2014/12/19 modify
			if (this.testhash!=null) {
				if(!this.testhash.isPreLoad()){
			...
			//nextSegmentIndex=0 rootName=/index.jsp/%FD%05I%2A%B63%D3  /index.jsp/%FD%05I%2A%B63%D3/%00
			if(testhash!=null){
				String path =testhash.parsePath(rootName.toString(),3);
				...
				System.out.println("Segment 852 hashName:"+hashName+"  trieTreeName:"+trieTreeName);
				Vertex vertex = testhash.FindPath("/localhost/ROOT"+hashName, trieTreeName);
				if(vertex!=null)
				{
					synchronized(vertex){
						if(vertex.isExist()==true){
							vertex.getVertexJumpList().add(co);
							System.out.println("ContentObject name is "+co.name().toString());
						}
					}
				}
			}
	...
	protected long newBlock(ContentName rootName,
		...
		if(testhash!=null){
			...
			System.out.println("Segment 932 hashName:"+hashName+"  trieTreeName:"+trieTreeName);
			Vertex vertex = testhash.FindPath("/localhost/ROOT"+hashName, trieTreeName);
			if(vertex!=null)
			{
